    ...
    <div className="group relative overflow-hidden rounded-2xl bg-white border border-slate-200 hover:border-blue-500 transition-all duration-500 hover:shadow-2xl hover:shadow-blue-500/20">
      ...
      <div className="absolute bottom-0 left-0 right-0 p-8 text-white">
        ...
        {activity.disabled ? (
            <button
              disabled
              className="bg-gray-500 cursor-not-allowed text-white font-semibold py-3 px-8 rounded-3xl opacity-70"
            >
              Découvrir
            </button>
          ) : (
            <Link
              href={
                activity.hubDeJeuxId
                  ? `/hub-de-jeux#${activity.hubDeJeuxId}`
